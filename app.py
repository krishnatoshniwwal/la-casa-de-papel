...
# ── FUTURISTIC AVATARS — inline SVG data URIs (zero network load) ─────────────
...
def render_bg():
    ...
    function init(){
      ...
      for(let i=0;i<count;i++){
        const w = randBetween(22,55);
        const h = randBetween(60, H*0.52);
        const x = i*(W/count) + randBetween(-4,4);
        buildings.push({x,w,h,
          windows: Array.from({length: Math.floor(h/18)}, ()=>({
            on: Math.random()>0.35,
    ...
    function drawSkyline(){
      ...
      grd.addColorStop(1,'rgba(6,2,15,0)');
      ctx.fillStyle=grd;
    ...
    function drawParticles(){
      particles.forEach(p=>{
        ...
        ctx.globalAlpha=p.alpha*(0.6+0.4*Math.sin(Date.now()*0.003+p.x));
...
# ── ZONE → FLOOR MAPPING (kept from original) ──────────────────────────────────
ZONE_TO_FLOOR = {
    "LOBBY": "L1",
    "CASINO": "L0",
    "CASHIER_CAGE": "L0",
...
def render_floor_map(zone_key: str):
    ...
    # ── 1. Blurred backdrop in the real DOM ────────────────────────────────────
    st.markdown("""
    ...
    # ── 2. Map iframe — JS repositions it to fixed/centered modal ──────────────
    components.html(f"""
    ...
    (function() {{
      ...
      function recentre() {{
        ...
        if (main) {{
          ...
          var w    = Math.min(rect.width * 0.96, 960);
          f.style.left      = cx + 'px';
          f.style.width     = w + 'px';
    ...
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    html, body {{
      ...
      background: linear-gradient(140deg, #0a001e 0%, #07020f 100%);
    ...
    .modal-header {{
      ...
      background: linear-gradient(90deg, rgba(255,45,120,0.1), rgba(139,43,226,0.12));
      border-bottom: 1px solid rgba(255,45,120,0.22);
    }}
    .modal-title {{
      font-family: 'Orbitron', monospace; font-size: 0.82rem; font-weight: 900;
      letter-spacing: 0.2em; color: #ff2d78;
      text-shadow: 0 0 16px rgba(255,45,120,0.9);
    }}
    .modal-sub {{
      font-size: 0.68rem; color: rgba(0,212,255,0.75);
      letter-spacing: 0.07em; margin-top: 3px;
    }}
    .close-btn {{
      background: rgba(255,45,120,0.12); border: 1px solid rgba(255,45,120,0.55);
      color: #ff2d78; border-radius: 8px; padding: 8px 18px; cursor: pointer;
      font-family: 'Orbitron', monospace; font-size: 0.65rem; font-weight: 700;
      letter-spacing: 0.15em; transition: all 0.2s ease;
    }}
    .close-btn:hover {{
    ...
    .map-grid {{
      ...
      grid-template-columns: repeat({cols}, minmax(72px, 1fr));
    }}
    .cell {{
      min-height: 62px; border-radius: 10px;
      display: flex; flex-direction: column;
      align-items: center; justify-content: center;
      text-align: center; padding: 6px 4px; position: relative;
    }}
    ...
    .you-badge  {{
      ...
      font-size: 6.5px; color: #00ff88; letter-spacing: 0.06em;
    ...
    .you            {{ background: rgba(0,255,136,0.13);  border: 2px solid rgba(0,255,136,0.85);  color: #ccffe8;
                       box-shadow: 0 0 22px rgba(0,255,136,0.35); animation: youPulse 2s ease-in-out infinite; }}
    @keyframes youPulse {{
      0%,100% {{ box-shadow: 0 0 22px rgba(0,255,136,0.35); }}
      50%      {{ box-shadow: 0 0 38px rgba(0,255,136,0.65); }}
    }}
    ...
    .vault          {{ background: rgba(255,215,0,0.18);   border: 1px solid rgba(255,215,0,0.75);  color: #ffd700;
                       animation: vaultGlow 2.5s ease-in-out infinite; }}
    @keyframes vaultGlow {{
    ...
    .you-leg   {{ color: #00ff88; }}
    .zone-leg  {{ color: #00d4ff; }}
    .vault-leg {{ color: #ffd700; }}
    .elev-leg  {{ color: #a78bfa; }}
    .shaft-leg {{ color: #2dd4bf; }}
    </style>
...
if st.session_state.game_over:
    ...
    <div style="background:linear-gradient(135deg,rgba(255,0,60,0.15),rgba(80,0,0,0.25));
      ...
      <div style="font-family:'Orbitron',monospace; font-size:1.5rem; font-weight:900;
                  color:#ff2d78; letter-spacing:0.2em;
                  text-shadow:0 0 30px rgba(255,45,120,1);">
        🚨 OPERATIVE CAPTURED
      </div>
    ...
    if msg["role"] == "assistant":
        with st.chat_message("assistant", avatar=ORACLE_AVATAR):
            ...
            <div style="background:linear-gradient(135deg,rgba(139,43,226,0.1),rgba(0,212,255,0.06));
                        ...
                        padding:14px 18px; margin:2px 0;
                        box-shadow:0 0 20px rgba(139,43,226,0.12);">
